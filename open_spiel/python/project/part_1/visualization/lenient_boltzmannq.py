...
for i, K in zip(range(4),[1,5,10,float("inf")]):
    dyn_prisoners_dilemma = dynamics_lb.MultiPopulationDynamicsLB(payoff_matrix_prisoners_dilemma, [partial(
        dynamics_lb.lenient_boltzmann, temperature=0.01, K=K)] * 2)
...
for i, K in zip(range(4),[1,7,20,float("inf")]):
    dyn_matching_pennies = dynamics_lb.MultiPopulationDynamicsLB(payoff_matrix_matching_pennies, [partial(
        dynamics_lb.lenient_boltzmann, temperature=0.01, K=K)] * 2)
...
for i, K in zip(range(4),[1,3,10,float("inf")]):
    dyn_battle_of_the_sexes = dynamics_lb.MultiPopulationDynamicsLB(payoff_matrix_battle_of_the_sexes, [partial(
        dynamics_lb.lenient_boltzmann, temperature=0.01, K=K)] * 2)
...
for i, K in zip(range(4),[1,5,10,float("inf")]):
    dyn_rock_paper_scissors = dynamics_lb.SinglePopulationDynamicsLB(payoff_matrix_rock_paper_scissors, partial(
        dynamics_lb.lenient_boltzmann, temperature=0.01, K=K))
